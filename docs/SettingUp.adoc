...
. Run the `studybuddy.Main` and try a few commands
...
After forking the repo, the documentation will still have the DRAGANDDROP branding and refer to the `draganddrop/studybuddy` repo.

If you plan to develop this fork as a separate product (i.e. instead of contributing to `draganddrop-studybuddy`), you should do the following:
...
When you are ready to start coding, we recommend that you get some sense of the overall design by reading about <<DeveloperGuide#Design-Architecture, Study Buddy's architecture>>.
