...
+
image::basic/clear/clear_result.png[width="700", title="Clear result"]
...
. Study Buddy should respond with:
+
image::basic/deleteduplicate/delete_dup_confirm.png[width="600", title="Response to 'delete duplicates'"]

. Press kbd:[enter] again to confirm
. Duplicate tasks have been deleted! You will notice that only one copy (most recent) of each task will be left in the list.
+
image::basic/deleteduplicate/delete_dup_result.png[width="600", title="Updated list (without duplicate tasks)"]
...
+
image::basic/filter/filter_type_result.png[width="600", title="Response to filter by task type 'assignment'"]
