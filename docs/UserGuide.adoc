...
image::dashboard/dashboard_1.png[width="600", title = "Menu bar"]

==== All Tasks Panel

By default, the *All Tasks Panel* lists all unarchived tasks record with their detail information in their creation order.

You can::
* Reorder them using command <<Sort>>.
* Filter them using command <<Filter>> or <<Find>>.

image::dashboard/dashboard_2.png[width="600", title = "All tasks panel"]

===== Task Badges
To bold the important information of each tasks in *All Tasks Panel* , we use *Badges* to highlight `Module Code`,
`Task Type` and  `Task Status` of each tasks.
...
image::dashboard/dashboard_4.png[width="600", title = "Interactive Prompt Box"]
