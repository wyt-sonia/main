...
Instead of forcing you to remember long wordy and easy-typo commands,
Study Buddy *provides an interactive way of communication*, which not only improves
...
+
. The box with hint `Please enter your command here...` is the entry for your commands. You can key in your desired command
here and pressing kbd:[Enter] to execute.
. To check the available command, you can key in *`help`* and pressing kbd:[Enter] and you are also
welcome to refer to <<Features>> for details of each command.
...
image::calender_init.png[width="790", align= "left"]
