...
.Sequence Diagram for Add to Due Soon list
image::AddDueSoonSequenceDiagram.png[width = "200", length = "300"]
{bl}

2. When user deletes a task:
...
3. When user refreshes due soon list:
...
... If there are no errors, the RefreshCommand object is created which iterates through the main list and calls addDueSoonTask() in Model, for any tasks that are due soon and do not already exist in due soon list.
+
...
4. When the application launches:
